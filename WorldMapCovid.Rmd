...
```{r}
content <- paste("<strong>", WorldMapCovid$name, "</strong>", "<br/>", WorldMapCovid$CumulConfirmes, "Cas")

content2 <- paste("<strong>", WorldMapCovid$name, "</strong>","<br/>", WorldMapCovid$CumulMorts, "Décés")

content3 <- paste("<strong>", WorldMapCovid$name, "</strong>", "<br/>", WorldMapCovid$CumulSoignes, "Soignés")
...
leaflet(WorldMapCovid) %>% 
  ...
  addCircleMarkers(radius = ~sqrt(category2)*5, popup = content2,
    color = "red",
    stroke = FALSE, fillOpacity = 0.4) %>% 
  addCircleMarkers(radius = ~sqrt(category3)*5, popup = content3,
    color = "green",
    stroke = FALSE, fillOpacity = 0.4)
```
...
Test pour commit sur branche CodeVal